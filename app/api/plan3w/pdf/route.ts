...
import { buildPlan3wPdf, type Plan3wItem } from "@/lib/plan3wPdf";
...
export async function GET(request: Request) {
  ...
  const now = new Date();
  const pdf = await buildPlan3wPdf({
  ...
  return new Response(pdf as BodyInit, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="Plan-3W-${familia.id}-${nombre}.pdf"`,
