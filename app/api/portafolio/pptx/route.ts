...
import { TODAS_PREGUNTAS, type RespuestasVerif } from "@/app/verificacion/data";
import { aplicaAlGiro } from "@/app/verificacion/aplicabilidad";
import type { SeccionEvid, ImagenEvid } from "@/lib/portafolioPdf";
import { buildPortafolioPptx } from "@/lib/portafolioPptx";
...
export async function GET(request: Request) {
  ...
  for (const p of TODAS_PREGUNTAS) {
    ...
    secciones.push({ codigo: p.codigo, tipo: p.tipo, texto: p.texto, descripcion, imagenes });
  }

  const pptx = await buildPortafolioPptx({
    empresa: (doc.empresa?.razonSocial as string) || "Establecimiento",
    giro: giro || "",
    lugar:
      [doc.empresa?.municipio, doc.empresa?.estado].filter(Boolean).join(", ") || "",
    fecha: new Date(
      verif.revisadoEn || doc.actualizadoEn || doc.creadoEn || Date.now(),
  ...
  return new Response(pptx as BodyInit, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="Portafolio-de-Evidencias-${nombre}.pptx"`,
